...
- **D2.6 (2026-08-08)** — AI 검색 브리지. 일지 `docs/daily/2026-08-08-phase-d2.6.md`,
  ...
  - 고민 입력 → 질의 조립 → **기본 브라우저**로 구글 AI 모드(`udm=50`). 앱은 결과를 읽지 않는다
  ...
  - vitest **104개** (+ routineParse 20 / aiQuery 9)
...
- **D2.6 시각 확인 3건.** ① 「🌐 브라우저에서 검색」이 실제로 기본 브라우저에서 구글 AI
  모드를 여는가 ② **실물** AI 답변을 붙여넣었을 때 파싱이 통하는가(고정 샘플로만 검증했다)
  ③ 펼친 상태의 「AI로 루틴 찾기」 레이아웃 (접힌 상태·AI 태그는 캡처로 확인)
