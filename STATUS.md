...
**마지막 갱신: 2026-08-09** · **현재 Phase: D2.7 완료(자동 검증까지) → 다음 D3**
...
- **검증이 오늘 통계에 세션 4건(각 ~4분, `🧪q1`/`🧪q2`)을 섞었다.** dev 와 설치본이 같은 DB 를
  쓰기 때문. 지우고 싶으면 검증 전 DB 사본이 스크래치패드에 있다
  (`…/scratchpad/db-backup-pre-d2.7`, 2026-08-09 16:02 시점)
