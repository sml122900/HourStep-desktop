...
- **상태를 바꾸는 `--debug-cmd` 명령을 `wait` 없이 연달아 보내지 말 것.** 전부 오버레이
  웹뷰로의 비동기 이벤트 왕복이라 **도착·처리 순서가 보장되지 않는다** — 뒤에 보낸
  명령의 응답이 앞선 명령보다 먼저 찍힐 수 있다. D2.10 검증에서 `action-toggle` 을
  `wait` 없이 여러 번 연달아 보냈다가 실제로 순서가 뒤바뀌었고, 그 스크립트에 섞여 있던
  `start-session`/`end-session` 때문에 세션 1건이 실 데이터에 남아 직접 지워야 했다
  (`docs/daily/2026-08-12-phase-d2.10.md`). 상태를 바꾸는 명령 사이에는 매번 `wait` 를
  넣고, 읽기만 하는 명령(`dump`/`*-dump`)도 직전 쓰기 명령의 응답을 봐야 신뢰할 수 있다
