...
- **D1 착수 전 남은 사용자 확인** (자동화 불가 — 검증 정책상 지각 판단·재부팅):
  - **C-1 / C-3~C-5** 전체화면 위 표시, 특히 **배타적 전체화면 게임** (D3 설계 입력)
  - **E-4'** 트레이 아이콘이 눈에서 사라지는가
  - **A** 재부팅 후 자동 상주
  - **B-2' / B-4b** 실제 마우스 클릭 전달·포커스 유지 → Windows Sandbox/VM E2E 몫
- **미결 결정**: 오버레이 투명 영역 클릭 차단 수정을 D1에 넣을지 D3로 미룰지 (C 결과 보고 판단)
